...
class c_bahan extends CI_Controller {
    ...
    function tampil_tambah_bahan(){
        $data['satuan'] = $this->m_bahan->edit_satuan()->result();
        $this->load->view('v_tambah_bahan.php', $data); 
    }
    ...
    if($checks !=""){
        foreach($checks as $a){
            $data = array(
                ...
                "golongan_biaya" => $this->input->post("golBahan".$a),
                "status" => 1
            );
    ...
    function edit_bahan($idbahan){
        ...
        $data['edit_satuan'] = $this->m_bahan->edit_satuan()->result();
